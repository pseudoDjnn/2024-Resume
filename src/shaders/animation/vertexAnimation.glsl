...
// Audio levels used with perlin noise
  ...
  float displacementInteger = fract(sin(uAudioFrequency * 1000.0)) * (noise / 13.0);
  ...
  modelPosition.y += elevation * (generateNoise * 0.1);
  ...
  modelPosition.x /= sin(aRandom * uFrequency.x - uTime * smoothstep(-0.144, 0.987, -uAudioFrequency * 0.002) * 0.2) * -21.8;
  modelPosition.z /= cos(aRandom * uFrequency.y - uTimeAnimation * smoothstep(-0.212, 1.188, uAudioFrequency * 0.005)) * 13.21;
  ...
  modelPosition.z += (random2D(modelPosition.zx * generateNoise + uTime) - 0.5) * stuttering;
